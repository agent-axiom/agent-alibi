...
test("solo match starts and reaches final case file", async ({ page }) => {
  ...
  await expect(page.getByLabel(/score popup/i).getByText(/\+2 escape bonus/i)).toBeVisible();
  await expect(page.getByLabel(/score popup/i).getByText(/afterburner \+1/i)).toBeVisible();
...
test("first steal cashout surge owns the center stage", async ({ page }) => {
  ...
  const overlayState = await page.evaluate(() => {
    const centerBand = {
      ...
      right: innerWidth * 0.75,
      top: innerHeight * 0.12,
      bottom: innerHeight * 0.58
    };
    const overlays = [
      ["surge", "[aria-label=\"Cashout surge\"]"],
      ["score", "[aria-label=\"Score popup\"]"],
      ["banner", "[aria-label=\"Objective banner\"]"],
      ["route", "[aria-label=\"Route pulse\"]"],
      ["spotlight", ".arcade-spotlight"]
    ...
    const inCenter = visible.filter(
      (overlay) =>
        overlay.rect.left < centerBand.right &&
        overlay.rect.right > centerBand.left &&
        overlay.rect.top < centerBand.bottom &&
        overlay.rect.bottom > centerBand.top
    );
    return { visibleNames: visible.map((overlay) => overlay.name), centerNames: inCenter.map((overlay) => overlay.name) };
  });
  expect(overlayState.visibleNames).toEqual(["surge"]);
  expect(overlayState.centerNames).toEqual(["surge"]);
  await expect(page.getByLabel(/score popup/i)).toBeHidden();
  await expect(page.getByLabel(/objective banner/i)).toBeHidden();
  await expect(page.getByLabel(/route pulse/i)).toBeHidden();
  await expect(page.locator(".arcade-spotlight")).toBeHidden();
...
test("rival steals trigger a clear red loot alert", async ({ page }) => {
  ...
  await expect(page.getByLabel(/active action/i).getByText(/recover moon pearl \+3 \/ swing \+6/i)).toBeVisible();
...
test("rival carrier near cashout triggers an imminent warning", async ({ page }) => {
  ...
  const emergency = page.getByLabel(/rival cashout emergency/i);
  await expect(emergency.getByText(/red cashout imminent/i)).toBeVisible();
  await expect(emergency.getByText(/intercept now/i)).toBeVisible();
  await expect(emergency.getByText(/deny red \+3 \/ swing \+6/i)).toBeVisible();
  await expect(emergency.getByText(/rook is \d+m from lift/i)).toBeVisible();
  await expect(page.getByLabel(/objective banner/i)).toBeHidden();
  const currentObjective = page.getByLabel(/current objective/i);
  await expect(currentObjective.getByText("Red cashout imminent", { exact: true })).toBeVisible();
  await expect(currentObjective.getByText(/intercept rook before lift/i)).toBeVisible();
  await expect(page.getByLabel(/active action/i).getByText(/close gap \d+m/i)).toBeVisible();
  const objectiveCompass = page.getByLabel(/objective compass/i);
  await expect(objectiveCompass.getByText(/intercept/i)).toBeVisible();
  await expect(objectiveCompass.getByText(/rook \+3/i)).toBeVisible();
  await expect(objectiveCompass.getByText("Deny Red +3 / swing +6", { exact: true })).toBeVisible();
  await expect(objectiveCompass.getByText(/rook scan/i)).toBeHidden();
  const contractChain = page.getByLabel(/contract chain/i);
